...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            C3Ghost, nn.Conv2d, nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3, PSA, SCDown, C2fCIB, C2f_Faster, C2f_ODConv, Enhanced_PSA_With_SimAM,
            ...
            MANet_Pzconv, C2f_MANet, MANet_GS, PzSPPF, PSEMA , PSimAM, HybridAttention, DSConv, shuffle_SE_1, shuffle_SE_2,
            ...
            C2f_MambaVision, FourierConv, C2f_FourierConv, C2f_GLVSS, C2f_SENetV2
        }:
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            c2 = m.channel
                   ...
                   DySample, CARAFE, CAA, ELA, CAFM, AFGCAttention, EUCB, ContrastDrivenFeatureAggregation, FSA, DualSimAM_SE,DualSimAM_RelativeEnhance,SimAMAttention,MultiScaleSimAMAttention,HybridSimAMAttention,
                   AttentiveLayer, EUCB_SC}:
            c2 = ch[f]
            args = [c2, *args]
            # print(args)
        elif m in {SimAM, SpatialGroupEnhance, DualSimAM }:
            c2 = ch[f]
        ...
        #     args = [c2, *args]
            ...
            args = []
            c2 = ch[f[0]]  # 输出通道数与输入相同（因为直接加法）
        elif m in [shuffle_SE_1, shuffle_SE_2]:
            # 获取输入通道数
            c1 = ch[f] if isinstance(f, int) else [ch[x] for x in f]
            c1 = c1[0] if isinstance(c1, list) else c1

            # 获取输出通道数
            if m is shuffle_SE_1:
                # shuffle_SE_1: 输入输出通道数相同
                c2 = c1
            else:  # shuffle_SE_2
                # shuffle_SE_2: 从args获取输出通道数，如果没有则使用输入通道数
                c2 = args[0] if args else c1 * 2  # 默认输出通道翻倍
