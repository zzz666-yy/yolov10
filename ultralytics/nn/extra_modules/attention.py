...
__all__ = ['EMA','PSEMA', 'SimAM','PSimAM', 'SimAMAttention','MultiScaleSimAMAttention','DualSimAM','DualSimAM_SE','DualSimAM_RelativeEnhance','Enhanced_PSA_With_SimAM','HybridSimAMAttention','SpatialGroupEnhance', 'BiLevelRoutingAttention', 'BiLevelRoutingAttention_nchw', 'TripletAttention',
...
class DualSimAM(nn.Module):
    ...
    def __init__(self, e_lambda=1e-4, fusion_method='additive'):
        """
        初始化双维度SimAM

        Args:
            e_lambda: 防止除零的小常数
            fusion_method: 融合方法，可选 'adaptive', 'multiplicative', 'additive'
        """
        super(DualSimAM, self).__init__()
    ...
    def __repr__(self):
        return f'DualSimAM(lambda={self.e_lambda}, fusion={self.fusion_method})'
    ...
    def forward(self, x):
        ...
        # 计算空间和通道维度的基础值
        n_spatial = height * width - 1  # 空间自由度
        n_channel = channels - 1  # 通道自由度
        ...
        # 计算与空间均值的平方差 [b, c, h, w]
        spatial_var_term = (x - spatial_mean).pow(2)

        # 按照原始SimAM公式计算空间能量函数 [b, c, h, w]
        spatial_energy = spatial_var_term / (
                    4 * (spatial_var_term.sum(dim=[2, 3], keepdim=True) / n_spatial + self.e_lambda)) + 0.5

        # 空间注意力权重 [b, c, h, w]
        spatial_attention = self.sigmoid(spatial_energy)

        # ==================== 通道注意力计算 ====================
        # 计算每个空间位置的通道均值 [b, 1, h, w]
        channel_mean = x.mean(dim=1, keepdim=True)

        # 计算与通道均值的平方差 [b, c, h, w]
        channel_var_term = (x - channel_mean).pow(2)

        # 按照原始SimAM公式计算通道能量函数 [b, c, h, w]
        channel_energy = channel_var_term / (
                    4 * (channel_var_term.sum(dim=1, keepdim=True) / n_channel + self.e_lambda)) + 0.5

        # 通道注意力权重 [b, c, h, w]
        channel_attention = self.sigmoid(channel_energy)

        # ==================== 注意力融合 ====================
        if self.fusion_method == 'multiplicative':
            # 乘法融合：强调重要区域
            combined_attention = spatial_attention * channel_attention

        elif self.fusion_method == 'additive':
            # 加法融合：平衡两种注意力
            combined_attention = (spatial_attention + channel_attention) / 2

        elif self.fusion_method == 'adaptive':
            # 自适应融合：可学习权重平衡
            total_weight = torch.abs(self.spatial_weight) + torch.abs(self.channel_weight)
            combined_attention = (torch.abs(self.spatial_weight) * spatial_attention +
                                  torch.abs(self.channel_weight) * channel_attention) / total_weight

        else:
            raise ValueError(f"Unknown fusion method: {self.fusion_method}")

        # 应用注意力权重
        output = x * combined_attention

        return output



class DualSimAM_SE(nn.Module):
    """
    双维度SimAM注意力机制（使用SE注意力作为通道维度）
    先进行通道注意力（SE），再进行空间注意力（SimAM）
    无融合部分，直接串联应用
    """

    def __init__(self, channel=512, reduction=16, e_lambda=1e-4):
        """
        初始化双维度SimAM-SE

        Args:
            channel: 输入通道数
            reduction: SE注意力中的降维比例
            e_lambda: 防止除零的小常数
        """
        super(DualSimAM_SE, self).__init__()

        self.e_lambda = e_lambda

        # SE注意力模块用于通道维度
        self.se_attention = SEAttention(channel=channel, reduction=reduction)

        # Sigmoid激活函数
        self.sigmoid = nn.Sigmoid()

    def __repr__(self):
        return f'DualSimAM_SE(lambda={self.e_lambda})'
    ...
    def forward(self, x):
        ...
        # 计算空间自由度
        n_spatial = height * width - 1

        # ==================== 通道注意力计算（使用SE） ====================
        # 应用SE注意力得到通道增强的特征
        channel_enhanced = self.se_attention(x)

        # ==================== 空间注意力计算（使用SimAM） ====================
        # 计算每个通道的空间均值 [b, c, 1, 1]
        spatial_mean = channel_enhanced.mean(dim=[2, 3], keepdim=True)

        # 计算与空间均值的平方差 [b, c, h, w]
        spatial_var_term = (channel_enhanced - spatial_mean).pow(2)
        ...
        # 应用空间注意力权重
        output = channel_enhanced * spatial_attention
...
class SimAMAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        返回:
            加权后的特征图 [batch_size, channels, height, width]
        """
        ...
        # 计算局部方差项
        local_var_term = (x - local_mean).pow(2)

        # 使用平均池化计算局部方差 (5x5区域)
        local_var = F.avg_pool2d(local_var_term, kernel_size=5, stride=1, padding=2)

        # 计算能量函数
        # 分母中的 + self.e_lambda 用于数值稳定性
        spatial_energy = local_var_term / (4 * (local_var + self.e_lambda)) + 0.5

        # 应用sigmoid得到注意力权重
        attention_weights = self.sigmoid(spatial_energy)
...
class MultiScaleSimAMAttention(nn.Module):
    def __init__(self, e_lambda=1e-4):
        super(MultiScaleSimAMAttention, self).__init__()
        self.e_lambda = e_lambda

    def compute_energy(self, x, kernel_size):
        padding = kernel_size // 2
        local_mean = F.avg_pool2d(x, kernel_size=kernel_size, stride=1, padding=padding)
        local_var_term = (x - local_mean).pow(2)
        local_var = F.avg_pool2d(local_var_term, kernel_size=kernel_size, stride=1, padding=padding)
        energy = local_var_term / (4 * (local_var + self.e_lambda)) + 0.5
        return torch.sigmoid(energy)

    def forward(self, x):
        # 多尺度注意力
        attn_3x3 = self.compute_energy(x, 3)
        attn_5x5 = self.compute_energy(x, 5)
        attn_7x7 = self.compute_energy(x, 7)

        # 加权融合
        combined_attn = (attn_3x3 + attn_5x5 + attn_7x7) / 3
        return x * combined_attn


class HybridSimAMAttention(nn.Module):
    """
    混合SimAM注意力模块：结合5x5局部注意力和全局注意力
    采用可学习融合权重和残差连接保护原始信息

    合理性分析：
    1. 局部注意力捕获细节特征和空间关系
    2. 全局注意力理解整体上下文和语义信息
    3. 可学习权重自适应调整局部全局比例
    4. 残差连接确保原始信息不丢失，提高训练稳定性
    """
    ...
    def __init__(self, e_lambda=1e-4):
        ...
        参数:
            e_lambda: 数值稳定性参数，防止除零错误
        """
        super(HybridSimAMAttention, self).__init__()
        self.e_lambda = e_lambda
        # 可学习的融合权重参数，初始值为0.5（平等对待局部和全局）
    ...
    def forward(self, x):
        ...
        返回:
            output: 加权后的特征图 [batch_size, channels, height, width]
        """
        # 保存原始输入特征，用于残差连接
        residual = x
        ...
        # 计算局部邻域的方差
        local_var = F.avg_pool2d(local_var_term, kernel_size=5, stride=1, padding=2)
        # 计算局部能量函数：衡量当前位置与局部邻域的差异程度
        local_energy = local_var_term / (4 * (local_var + self.e_lambda)) + 0.5
        # 通过sigmoid将能量值转换为注意力权重(0-1)
        local_weights = self.sigmoid(local_energy)

        # ==================== 全局注意力计算 ====================
        n = height * width - 1  # 空间位置总数-1（用于归一化）
        # 计算全局空间均值
        global_mean = x.mean(dim=[2, 3], keepdim=True)
        # 计算当前位置与全局均值的平方差
        global_var_term = (x - global_mean).pow(2)
        # 计算全局能量函数：衡量当前位置与全局均值的差异程度
        global_energy = global_var_term / (
                    4 * (global_var_term.sum(dim=[2, 3], keepdim=True) / n + self.e_lambda)) + 0.5
        # 通过sigmoid将能量值转换为注意力权重(0-1)
        global_weights = self.sigmoid(global_energy)

        # ==================== 可学习融合 ====================
        # 使用sigmoid确保融合权重在0-1之间
        alpha = torch.sigmoid(self.fusion_weight)
        # 混合局部和全局注意力权重
        mixed_attention_weights = alpha * local_weights + (1 - alpha) * global_weights

        # ==================== 应用注意力和残差连接 ====================
        # 应用混合注意力权重到输入特征
        attended_features = x * mixed_attention_weights
        # 残差连接：将注意力增强的特征与原始特征相加
        # 确保即使注意力效果不佳，原始信息仍然保留
        output = residual + attended_features

        return output


class DualSimAM_RelativeEnhance(nn.Module):
    """
    双维度SimAM注意力机制（相对增强方式）
    通过相对比较来增强重要信息，而不是抑制不重要信息
    """

    def __init__(self, e_lambda=1e-4, fusion_method='additive'):
        super(DualSimAM_RelativeEnhance, self).__init__()
    ...
    def __repr__(self):
        return f'DualSimAM_RelativeEnhance(lambda={self.e_lambda}, fusion={self.fusion_method})'

    def forward(self, x):
        batch_size, channels, height, width = x.size()

        # ==================== 相对增强的空间注意力 ====================
        # 重塑特征以便进行空间维度的softmax
        spatial_reshaped = x.view(batch_size, channels, -1)  # [b, c, h*w]

        # 计算每个空间位置相对于其他位置的重要性
        spatial_importance = self.softmax(spatial_reshaped)

        # 将权重重塑回原始形状
        spatial_attention = spatial_importance.view(batch_size, channels, height, width)

        # 相对增强：将重要性权重缩放到[0.5, 1.5]范围
        spatial_attention = 0.5 + spatial_attention

        # ==================== 相对增强的通道注意力 ====================
        # 重塑特征以便进行通道维度的softmax
        channel_reshaped = x.view(batch_size, channels, -1).transpose(1, 2)  # [b, h*w, c]

        # 计算每个通道相对于其他通道的重要性
        channel_importance = self.softmax(channel_reshaped)

        # 将权重重塑回原始形状
        channel_attention = channel_importance.transpose(1, 2).view(batch_size, channels, height, width)

        # 相对增强：将重要性权重缩放到[0.5, 1.5]范围
        channel_attention = 0.5 + channel_attention
...
class SimAMGuidedAttention(nn.Module):
    def __init__(self, dim, num_heads=8, attn_ratio=0.5, simam_lambda=1e-4):
        super().__init__()

        # 安全性检查：确保num_heads至少为1
        ...
        # 安全的scale计算，避免除零错误
        self.scale = 1.0 / math.sqrt(self.key_dim) if self.key_dim > 0 else 1.0
        ...
        self.gate_conv = Conv(dim, self.num_heads, 1, act=False)
    ...
    def forward(self, x):
        ...
        # 检查维度是否匹配
        if C != self.qkv.conv.in_channels:
            raise ValueError(f"Input channels {C} don't match expected {self.qkv.conv.in_channels}")
        ...
        # 使用SimAM生成空间重要性图来引导注意力
        simam_weights = self.simam(x)  # [B, C, H, W]
        gate_weights = self.gate_conv(simam_weights)  # [B, num_heads, H, W]
        gate_weights = gate_weights.view(B, self.num_heads, N).unsqueeze(-1)  # [B, num_heads, N, 1]

        # 使用门控机制调整注意力权重
        guided_attn = attn * gate_weights.sigmoid()
        # 添加小数值避免除零错误
        guided_attn = guided_attn / (guided_attn.sum(dim=-1, keepdim=True) + 1e-8)

        x = (v @ guided_attn.transpose(-2, -1)).view(B, C, H, W) + self.pe(v.reshape(B, C, H, W))
        x = self.proj(x)
        return x


class Enhanced_PSA_With_SimAM(nn.Module):
    def __init__(self, c1, c2, e=0.5, simam_lambda=1e-4):
        super().__init__()
        assert c1 == c2

        # 确保c至少为1
        self.c = max(1, int(c1 * e))

        # 安全的num_heads计算，确保至少为1
        min_head_size = 32  # 设置最小head维度
        num_heads = max(1, self.c // min_head_size)

        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv(2 * self.c, c1, 1)

        # 使用改进的Attention模块
        self.attn = SimAMGuidedAttention(
            self.c,
            attn_ratio=0.5,
            num_heads=num_heads,  # 使用安全的num_heads
