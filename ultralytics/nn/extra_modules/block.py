...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C2f_Faster', 'C3_Faster', 'C3_ODConv', 'C2f_ODConv', 'Partial_conv3', 'C2f_Faster_EMA', 'C3_Faster_EMA', 'C2f_DBB',
           ...
           'C2f_CAMixer', 'HyperComputeModule', 'MANet', 'MANet_FasterBlock', 'MANet_FasterCGLU', 'MANet_Star','MANet_GS', 'MultiScaleGatedAttn', 'C2f_HFERB', 'C2f_DTAB', 'C2f_ETB', 'C2f_JDPM', 'WFU', 'PSConv', 'C2f_AP', 'ContrastDrivenFeatureAggregation',
           ...
           'C2f_GroupMambaBlock', 'C2f_MambaVision', 'PST', 'FourierConv', 'C2f_FourierConv', 'C2f_GLVSS', 'C2f_SENetV2', 'shuffle_SE_1', 'shuffle_SE_2', 'DWConvBlock',
           ]
...
class MANet_GS(nn.Module):
    """
    结合 GSConv 和 GSBottleneck 的 MANet 模块
    """

    def __init__(self, c1, c2, n=1, shortcut=False, p=1, kernel_size=3, g=1, e=0.5):
        super().__init__()
        self.c = int(c2 * e)  # 中间层通道数
        self.n = n  # Bottleneck 重复次数

        # 使用 GSConv 替代标准 Conv
        self.cv_first = GSConv(c1, 2 * self.c, 1, 1)
        self.cv_final = GSConv((4 + n) * self.c, c2, 1, 1)

        # 使用 GSBottleneck 替代标准 Bottleneck
        self.m = nn.ModuleList(
            GSBottleneck(self.c, self.c, k=3, e=1.0) for _ in range(n)
        )

        # 分支1：使用 GSConv
        self.cv_block_1 = GSConv(2 * self.c, self.c, 1, 1)

        # 分支2：保持原有的深度可分离卷积结构，但使用 GSConv 替代标准 Conv
        dim_hid = int(p * 2 * self.c)
        self.cv_block_2 = nn.Sequential(
            GSConv(2 * self.c, dim_hid, 1, 1),
            DWConv(dim_hid, dim_hid, kernel_size, 1),
            GSConv(dim_hid, self.c, 1, 1)
        )
    ...
    def forward(self, x):
        ...
        # 两个分支处理
        y0 = self.cv_block_1(y)  # 分支1：GSConv路径
        y1 = self.cv_block_2(y)  # 分支2：深度可分离路径
...
class EnhancedC2f(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__()
        self.c = int(c2 * e)
        self.cv1 = Conv(c1, 3 * self.c, 1, 1)  # 改为3个分支
        self.cv2 = Conv((3 + n) * self.c, c2, 1)

        # 三个不同的处理分支
        self.branch1 = Conv(self.c, self.c, 1, 1)  # 直接映射
        self.branch2 = DWConv(self.c, self.c, 3, 1)  # 深度卷积
        self.m = nn.ModuleList(Bottleneck(self.c, self.c, shortcut, g) for _ in range(n))

    def forward(self, x):
        y = list(self.cv1(x).chunk(3, 1))  # 分三块
        y[0] = self.branch1(y[0])  # 分支1处理
        y[1] = self.branch2(y[1])  # 分支2处理
        y.extend(m(y[-1]) for m in self.m)  # 分支3深度处理
        return self.cv2(torch.cat(y, 1))
...
class CCDown(nn.Module):
    def __init__(self, c1, c2, k, s):
        """
         Channel change Downsample (CCDown) module.
        ...
        """
        super().__init__()
        self.cv1 = Conv(c1, 2*c1, k, s)
        self.cv2 = Conv(2*c1, c2, k=1, s=1)
...
class shuffle_SE_1(nn.Module):
    """左图结构 (a): Channel Split → Conv → DWConv → Conv → SE → Concat → Channel Shuffle"""

    def __init__(self, c1, c2):
        super(shuffle_SE_1, self).__init__()
        self.c1 = c1
        self.c2 = c2
        ...
        # 分支1: identity branch (直接连接)
        # 分支2: 处理分支
        self.branch_conv1 = nn.Conv2d(split_channels, split_channels, 1, 1, 0, bias=False)
        self.branch_dwconv = nn.Conv2d(split_channels, split_channels, 3, 1, 1,
                                       groups=split_channels, bias=False)
        self.branch_conv2 = nn.Conv2d(split_channels, split_channels, 1, 1, 0, bias=False)
        self.branch_se = SEAttention(split_channels)

        # Channel Shuffle
        self.shuffle = ChannelShuffle(groups=2)

        # 合并BatchNorm层
        self.bn = nn.BatchNorm2d(split_channels)

    def forward(self, x):
        # Channel Split using chunk (固定50%分割)
        x1, x2 = torch.chunk(x, 2, dim=1)

        # 处理分支
        x2 = F.relu(self.bn(self.branch_conv1(x2)))
        x2 = self.bn(self.branch_dwconv(x2))
        x2 = self.bn(self.branch_conv2(x2))

        # SE模块
        x2 = self.branch_se(x2)
...
class shuffle_SE_2(nn.Module):
    """右图结构 (b): Conv → DWConv(stride=2) → Conv → SE → Concat → Channel Shuffle"""

    def __init__(self, c1, c2):
        super(shuffle_SE_2, self).__init__()
        self.c1 = c1
        self.c2 = c2

        # 主分支 (下采样分支)
        main_channels = c2 // 2
        self.main_conv1 = nn.Conv2d(c1, main_channels, 1, 1, 0, bias=False)
        self.main_dwconv = nn.Conv2d(main_channels, main_channels, 3, 2, 1,
                                     groups=main_channels, bias=False)
        self.main_conv2 = nn.Conv2d(main_channels, main_channels, 1, 1, 0, bias=False)
        self.main_se = SEAttention(main_channels)

        # 捷径分支 (下采样)
        self.shortcut = nn.Sequential(
            nn.Conv2d(c1, c1, 3, 2, 1, groups=c1, bias=False),
            nn.BatchNorm2d(c1),
            nn.Conv2d(c1, main_channels, 1, 1, 0, bias=False),
        ...
        # 合并BatchNorm层
        self.bn = nn.BatchNorm2d(main_channels)

    def forward(self, x):
        # 主分支
        x_main = F.relu(self.bn(self.main_conv1(x)))
        x_main = self.bn(self.main_dwconv(x_main))
        x_main = self.bn(self.main_conv2(x_main))

        # SE模块
        x_main = self.main_se(x_main)
...
class DWConvBlock(nn.Module):
    def __init__(self, c1, c2, stride=1,):
        ...
        # Depthwise Convolution
        self.dwconv = DWConv(c1, c1, 3, s=stride,)
