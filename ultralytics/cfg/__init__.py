    ...
    for k, v in cfg.items():
        if v is not None:  # None values may be from optional args
            if k in CFG_FLOAT_KEYS and not isinstance(v, (int, float)):
                ...
                cfg[k] = bool(v)
            # ===== 添加以下代码 =====
            # 添加对 tracker_type 的特殊处理 - 扩展允许的类型
            if k == "tracker_type" and v is not None:
                # 定义所有允许的跟踪器类型（包括自定义）
                allowed_trackers = [
                    'bytetrack',
                    'botsort',
                ...
                if v not in allowed_trackers and hard:
                    LOGGER.warning(
                        f"注意: 跟踪器类型 '{v}' 可能不是内置类型。"
                        f"确保已在 track.py 的 TRACKER_MAP 中注册。"
                    )
            # ===== 添加结束 =====
...
def check_dict_alignment(base: Dict, custom: Dict, e=None):
    ...
    # 扩展基础键集合以包含自定义跟踪器相关的键
    extended_base_keys = base_keys.copy()
    tracker_related_keys = {
        'tracker_type', 'gmc_method', 'gmc_downscale',
        'with_reid', 'feat_history', 'proximity_thresh', 'appearance_thresh',
        'kalman_filter', 'std_weight_position', 'std_weight_velocity'
    }
    extended_base_keys.update(tracker_related_keys)

    mismatched = [k for k in custom_keys if k not in extended_base_keys]
    if mismatched:
        from difflib import get_close_matches

        string = ""
        tracker_params_found = []
        other_params_found = []

        for x in mismatched:
            if x in tracker_related_keys:
                tracker_params_found.append(x)
            else:
                other_params_found.append(x)
                matches = get_close_matches(x, base_keys)
                matches = [f"{k}={base[k]}" if base.get(k) is not None else k for k in matches]
                match_str = f"Similar arguments are i.e. {matches}." if matches else ""
                string += f"'{colorstr('red', 'bold', x)}' is not a valid YOLO argument. {match_str}\n"

        # 如果是跟踪器相关参数且不在基础键中，只给警告而不是错误
        if tracker_params_found and not other_params_found:
            LOGGER.info(f"📌 检测到跟踪器特定参数: {tracker_params_found}，这些参数将被自定义跟踪器使用。")
        elif tracker_params_found and other_params_found:
            LOGGER.info(f"📌 检测到跟踪器特定参数: {tracker_params_found}，这些参数将被自定义跟踪器使用。")
