"""
byte_tracker_xywh.py - ByteTrack with XYWH Kalman Filter (like BoT-SORT)
Based on Ultralytics YOLO byte_tracker.py
支持可配置的 IOU 计算方式
"""
...
from .utils.kalman_filter import KalmanFilterXYWH  # 改为XYWH滤波器


class STrack(BaseTrack):
    """
    Single object tracking representation using Kalman filtering with XYWH state.
    修改：使用XYWH状态的卡尔曼滤波器（从BoT-SORT引入）
    ...
    def activate(self, kalman_filter, frame_id):
        ...
        # 注意：convert_coords现在返回xywh格式（中心x,中心y,宽,高）
...
class BYTETrackerXYWH:
    """
    BYTETracker with XYWH Kalman Filter (modified from BoT-SORT).
    保持ByteTrack的原始逻辑，但使用XYWH状态的卡尔曼滤波器。
    支持可配置的 IOU 计算方式（与 byte_tracker.py 保持一致）。
    """
    ...
    def __init__(self, args, frame_rate=30, iou_config=None):
        ...
        LOGGER.info(f"BYTETrackerXYWH initialized with iou_type={self.iou_type}, "
