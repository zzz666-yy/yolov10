"""
byte_tracker_gmc.py - ByteTrack with GMC motion compensation
Based on Ultralytics YOLO byte_tracker.py and bot_sort.py
"""
...
from .utils.kalman_filter import KalmanFilterXYAH  # 保持原XYAH滤波器
from .utils.gmc import GMC  # 从BoT-SORT引入GMC


class STrack(BaseTrack):
    """
    Single object tracking representation with GMC support.
    修改：添加GMC支持（从BoT-SORT引入）
    """
...
class BYTETrackerGMC:
    """
    BYTETracker with GMC motion compensation (modified from BoT-SORT).
    保持ByteTrack的原始逻辑和XYAH滤波器，但加入GMC运动补偿。
    """
    ...
    def get_kalmanfilter(self):
        """Returns a Kalman filter object with XYAH state."""
